...
class Cenario:
    ...
    def principal(self):
        ...
        label(frame=self.quadro ,pos=(-10,20,-5), text='NAO foi descrita pela trajetória dos peixes... ', height=14 , box=0)    
        ...
        while 1:
            if (fabs(blueFish.esqueleto.x - caixa.x) < 6) and ( fabs (blueFish.esqueleto.y - caixa.y) < 6 ):
                    ...
                    labelErrou = label(frame=self.quadro ,pos=(-10,10,-30), text='Você errou!O correto seria o círculo! ', height=14 , color=color.red)
