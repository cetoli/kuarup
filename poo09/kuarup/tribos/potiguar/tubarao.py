...
"""
#########################################
Tutubarão - o terror dos sete mares!
#########################################
...
"""
'''
Os tubarões brancos são perigosos...
'''
from visual import *
TITLE = 'Tutubarão!'
'''
Aqui carrega o cenário.... é onde instanciamos o ser marinho.
Não se esqueça: o professor mandou usar uma janela de 150(width) x 150(height)!
'''
class Cenario:
    def __init__(self):
    ...
    def principal(self):
        ...
        tubarao2 = Tubarao(escala =30, axis=(1,1,-2), pos= (-5,-3,20))  #Segundo peixinho
...
class Tubarao(SerMarinho):
    ...
    def desenha(self, escala=1):
        ...
        dente30 = cone(frame = self.esqueleto, pos=(16*escala,-5*escala,-2*escala), axis=(0,0.5*escala,0), radius=0.3*escala, color=color.white)
        ...
        guelra1 = cylinder (frame = self.esqueleto, pos=(6.*escala,0, 5.5*escala), axis=(0.1*escala,0, 0.1*escala),radius=3.*escala, color=color.gray(0.5))
        guelra2 = cylinder (frame = self.esqueleto, pos=(5.*escala,0, 5.5*escala), axis=(0.1*escala,0, 0.1*escala),radius=3.*escala, color=color.gray(0.5))
        guelra3 = cylinder (frame = self.esqueleto, pos=(4.*escala,0, 5.5*escala), axis=(0.1*escala,0, 0.1*escala),radius=3.*escala, color=color.gray(0.5))
        guelra4 = cylinder (frame = self.esqueleto, pos=(6.*escala,0,-5.5*escala), axis=(0.1*escala,0,-0.1*escala),radius=3.*escala, color=color.gray(0.5))
        guelra5 = cylinder (frame = self.esqueleto, pos=(5.*escala,0,-5.5*escala), axis=(0.1*escala,0,-0.1*escala),radius=3.*escala, color=color.gray(0.5))
        guelra6 = cylinder (frame = self.esqueleto, pos=(4.*escala,0,-5.5*escala), axis=(0.1*escala,0,-0.1*escala),radius=3.*escala, color=color.gray(0.5))
