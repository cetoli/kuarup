...
class Sea_turtle(Marine_life):
    def draw(self, esqueleto, pos=(0,0,0), scale=1., axis=[1,0,0]):
        ## Body
        self.casco1 = ellipsoid(pos=pos, length=5.*scale, height=0.7*scale, width=4.*scale, axis=(1,0.07,0), color=(0.4,0.4,0.4), material=self.get_texture("turtle_skin.jpg", width=256, height=256), frame=esqueleto)
        self.casco2 = ellipsoid(pos=(0,0.1,0), length=4.8*scale, height=1.5*scale, width=3.8*scale, axis=(1,0.07,0), color=(0.4,0.4,0.4), material=self.get_texture("turtle_skin.jpg", width=256, height=256), frame=esqueleto)
...
vs.scene = vs.display()
vs.scene.title = "Piraporangatu Turtle"
